...
export class ECSWorld implements World {
	...
	public static async create({
		...
		// init systems
		const systemInstances: System[] = systems.map((systemType) =>
			world.injectablesContainer.get(systemType)
		);
		...
		if (threads > 0) {
			...
			world.jobScheduler = new ECSJobScheduler(
				world.workerPool,
				world.deferredCommands
			);
	...
	public has<T extends ComponentType>(
		entity: Entity,
		componentType: T
	): boolean {
	...
	public get<T extends ComponentType>(
		entity: Entity,
		componentType: T
	...
	public detach<T extends ComponentType[]>(
		entity: Entity,
		componentTypes: T
	): void {
