...
const getConstructorDependenciesFromInjectable = <T>(
	target: Abstract<T>,
	parents: string[]
): Abstract<unknown>[] => {
	...
	if (dependencies.length < target.length) {
		throw new Error(
			`Class not decorated: ${[...parents, target.name].join(' -> ')}`
		);
	}
...
const getBase = <T extends TBase, TBase>(
	target: Abstract<T>
): Abstract<TBase> | undefined => {
	const base = Object.getPrototypeOf(target.prototype).constructor;

	return base === Object ? undefined : base;
};

export const getConstructorDependencies = <T>(
	target: Abstract<T>,
	parents: string[] = []
): Abstract<unknown>[] => {
	const dependencies = getConstructorDependenciesFromInjectable(
		target,
		parents
	);
