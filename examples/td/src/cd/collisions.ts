...
export const pointToCircle = (point: Point, circleCenter: Vector2, circleRadius: number): boolean =>
    distance(point, circleCenter) <= circleRadius;
...
export const rectangleToCircle = (
    ...
    let y = circleCenter.y;

    if (circleCenter.x < rectanglePosition.x) {
        x = rectanglePosition.x; // left edge
    } else if (circleCenter.x > rectanglePosition.x + rectangle.width) {
        x = rectanglePosition.x + rectangle.width; // right edge
    }
    if (circleCenter.y < rectanglePosition.y) {
        y = rectanglePosition.y; // top edge
    } else if (circleCenter.y > rectanglePosition.y + rectangle.height) {
        y = rectanglePosition.y + rectangle.height; // bottom edge
    }

    return pointToCircle({ x, y }, circleCenter, circleRadius);
};

export const circleToCircle = (positionA: Vector2, radiusA: number, positionB: Vector2, radiusB: number): boolean => {
    return distance(positionA, positionB) <= radiusA + radiusB;
};
